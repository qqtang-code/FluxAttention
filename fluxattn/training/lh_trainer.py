...
class Trainer(HFTrainer):
    ...
    def get_current_target_sparsity(
        ...
        for task in tasks:
            ...
            # Warmup logic: linear interpolation from start_sp to end_sp
            if global_step < self.num_sparsity_warmup_steps:
                progress = global_step / max(self.num_sparsity_warmup_steps, 1)
                sp = start_sp + (end_sp - start_sp) * progress
            else:
                sp = end_sp
