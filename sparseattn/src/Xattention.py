...
def Xattention_prefill_dim4(
    ...
    if mask.sum() == 0 or mask.sum() == num_heads:
        blockmask = approx_simple_mask[
            :, :, :max_q_block_num, :max_k_block_num
        ].contiguous()
    else:
        blockmask = approx_simple_mask[
    ...
    exact_streaming = True if head_mask_type.sum() != num_heads else False
    ...
    attn_output = block_sparse_attn_func(
        ...
        head_mask_type,
        streaming_info,
        blockmask if mask.sum() != 0 else None,
        max_q_len,
        max_k_len,
        p_dropout=0.0,
        deterministic=True,
